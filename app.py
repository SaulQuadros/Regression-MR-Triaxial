...
# Função para construir a equação em LaTeX com quebras de linha (a cada 4 termos)
def build_latex_equation(coefs, intercept, feature_names):
    terms_per_line = 4  # número de termos por linha
    eq_parts = [f"{intercept:.4f}"]
    # Gera cada termo com sinal adequado
    for coef, term in zip(coefs[1:], feature_names[1:]):
        sign = " + " if coef >= 0 else " - "
        eq_parts.append(sign + f"{abs(coef):.4f}" + term.replace(" ", ""))
    # Constrói a equação agrupando os termos em linhas
    lines = []
    current_line = "MR = " + eq_parts[0]
    for i, part in enumerate(eq_parts[1:], start=1):
        current_line += part
        if i % terms_per_line == 0 and i < len(eq_parts)-1:
            current_line += " \\\\ "  # quebra de linha no LaTeX
...
# tratando '^' para superscript e '~' para subscript
...
def plot_3d_surface(df, model, poly, energy_col):
    ...
    # Limita os valores: não negativos e não superiores ao máximo observado + desvio padrão
...
def generate_word_doc(equation_latex, metrics_text, fig, energy_type, degree):
    ...
    # Converte "σ_d" para "σ~d" para sinalizar que o "d" deve ser formatado como subíndice
...
if uploaded_file is not None:
    ...
    energy_col = "MR"  # Ajuste conforme necessário se a tabela tiver colunas diferentes (ex.: "MR_I", "MR_M").
