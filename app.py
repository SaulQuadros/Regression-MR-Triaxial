...
# A primeira linha inicia com "MR = <intercepto>", e as linhas subsequentes são agrupadas (4 termos por linha).
...
def plot_3d_surface(df, model, poly, energy_col):
    ...
    Xpg = poly.transform(Xg)
    MRg = model.predict(Xpg).reshape(s3g.shape)
    ...
    fig.update_layout(
        scene=dict(
            xaxis_title='σ₃ (MPa)',
            yaxis_title='σ<sub>d</sub> (MPa)',  # d como subscrito em HTML
            zaxis_title='MR (MPa)'
        ),
...
# Função para interpretar métricas estatísticas
def interpret_metrics(r2, r2_adj, rmse, mae, y):
    text = (
        f"**R²:** {r2:.6f}. Aproximadamente {r2*100:.2f}% da variabilidade de MR é explicada pelo modelo.\n\n"
        f"**R² Ajustado:** {r2_adj:.6f}. Penaliza variáveis extras, confirmando ausência de superajuste.\n\n"
        f"**RMSE:** {rmse:.4f} MPa. Erro quadrático médio indica desvio médio de {rmse:.4f} MPa.\n\n"
        f"**MAE:** {mae:.4f} MPa. Erro absoluto médio de {mae:.4f} MPa.\n\n"
        f"**Média de MR:** {y.mean():.4f} MPa.\n\n"
        f"**Desvio Padrão de MR:** {y.std():.4f} MPa.\n\n"
    )
    return text

# Função para gerar o documento Word com tudo
...
if uploaded_file:
    ...
    energy_type = st.sidebar.selectbox(
        "Tipo de energia", ["Normal","Intermediária","Modificada"], index=0
    )
    ...
    if st.button("Calcular"):
        ...
        n,p = len(y), Xp.shape[1] - (0 if model_type.startswith("s/") else 1)
        r2_adj = adjusted_r2(r2,n,p)
        rmse = np.sqrt(mean_squared_error(y,y_pred))
        mae  = mean_absolute_error(y,y_pred)

        eq_latex = build_latex_equation(reg.coef_, getattr(reg,"intercept_",0), poly.get_feature_names_out(["σ₃","σ_d"]))
        metrics_txt = interpret_metrics(r2,r2_adj,rmse,mae,y)

        st.write("### Equação de Regressão"); st.latex(eq_latex.strip("$$"))
        ...
        buf = generate_word_doc(eq_latex,metrics_txt,fig,energy_type,degree,getattr(reg,"intercept_",0),df)
        buf.seek(0)
