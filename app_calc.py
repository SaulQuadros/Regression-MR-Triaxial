...
def _wtz81(X, k1, k2, k3):
    s3, sd = X[:,0], X[:,1]
    theta = sd + 2*s3  # θ = σ1 + 2σ3, approximated as sd+2*s3?
...
def evaluate_quality(y, rmse, mae):
    ...
    return {
        "NRMSE_range": (nrmse, quality_label(nrmse,[0.05,0.10],labels_nrmse),
...
def calcular_modelo(df, model_type, degree):
    ...
    if model_type in CLASSICOS:
        ...
        eq="$$MR="+", ".join(f"{v:.4f}" for v in popt)+"$$"
        result.update({"eq_latex":eq,"intercept":0.0,"r2":r2,"r2_adj":r2_adj,
    ...
    if model_type.startswith("Polinomial"):
        ...
        Xp=poly.fit_transform(X); reg=LinearRegression(fit_intercept=(model_type=="Polinomial c/ Intercepto"))
        reg.fit(Xp,y); y_pred=reg.predict(Xp)
        r2=r2_score(y,y_pred); p_feat=Xp.shape[1]
        ...
        coefs=np.concatenate(([reg.intercept_],reg.coef_)) if reg.fit_intercept else reg.coef_
        eq=build_latex_equation(coefs,reg.intercept_,[""]+fnames)
        result.update({"eq_latex":eq,"intercept":reg.intercept_,"r2":r2,"r2_adj":r2_adj,
                       "rmse":rmse,"mae":mae,"mean_MR":y.mean(),"std_MR":y.std(),
                       "model_obj":reg,"poly_obj":poly,"is_power":False,"power_params":None})
    # Genérico Potência Composta
        ...
        y_pred=pot(X,*popt)
        r2=r2_score(y,y_pred); r2_adj=adjusted_r2(r2,len(y),len(popt))
        rmse=np.sqrt(mean_squared_error(y,y_pred)); mae=mean_absolute_error(y,y_pred)
        eq=f"$$MR = {popt[0]:.4f}σ₃^{{{popt[1]:.4f}}} + {popt[2]:.4f}(σ₃σ_d)^{{{popt[3]:.4f}}} + {popt[4]:.4f}σ_d^{{{popt[5]:.4f}}}$$"
        result.update({"eq_latex":eq,"intercept":0.0,"r2":r2,"r2_adj":r2_adj,
                       "rmse":rmse,"mae":mae,"mean_MR":y.mean(),"std_MR":y.std(),
                       "model_obj":pot,"poly_obj":None,"is_power":True,"power_params":popt})
    # Genérico Pezo
    else:
        def pezo(X_flat,k1,k2,k3):
            s3,sd=X_flat[:,0],X_flat[:,1]
            return k1*Pa*(s3/Pa)**k2*(sd/Pa)**k3
        popt,_=curve_fit(pezo,X,y,p0=[y.mean()/(Pa*(X[:,0]/Pa).mean()*(X[:,1]/Pa).mean()),1,1],maxfev=200000)
        y_pred=pezo(X,*popt)
        r2=r2_score(y,y_pred); r2_adj=adjusted_r2(r2,len(y),len(popt))
        rmse=np.sqrt(mean_squared_error(y,y_pred)); mae=mean_absolute_error(y,y_pred)
        const=popt[0]*Pa
        eq=f"$$MR = {const:.4f}(σ₃/Pa)^{{{popt[1]:.4f}}}(σ_d/Pa)^{{{popt[2]:.4f}}}$$"
        result.update({"eq_latex":eq,"intercept":0.0,"r2":r2,"r2_adj":r2_adj,
                       "rmse":rmse,"mae":mae,"mean_MR":y.mean(),"std_MR":y.std(),
                       "model_obj":pezo,"poly_obj":None,"is_power":True,"power_params":popt})
