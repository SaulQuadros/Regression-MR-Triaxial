...
def build_latex_equation(coefs, intercept, feature_names):
    """Monta equação LaTeX para modelo polinomial, quebrando em linhas de até 4 termos."""
    terms_per_line = 4
    parts = []
...
def evaluate_quality(y, rmse, mae):
    ...
    return {
        "NRMSE_range":    (nrmse,  quality_label(nrmse,  [0.05, 0.10], labels_nrmse),
                     "NRMSE: RMSE normalizado pela amplitude dos valores de MR; indicador associado ao RMSE."),
        ...
        "MAE %":    (mae_pct, quality_label(mae_pct, [0.10, 0.20], labels_mae),
...
def calcular_modelo(df, model_type, degree):
    ...
    if model_type.startswith("Polinomial"):
        ...
        r2_adj = adjusted_r2(r2, len(y), Xp.shape[1]) if len(y) > Xp.shape[1] + 1 else r2
        ...
        if fit_int:
            ...
            eq_latex = build_latex_equation(coefs, intercept, [""] + fnames)
        else:
            intercept = 0.0
        ...
        rmse = np.sqrt(mean_squared_error(y, y_pred))
        mae = mean_absolute_error(y, y_pred)
        eq_latex = f"$$MR = {popt[0]:.4f}σ₃^{{{popt[1]:.4f}}} + {popt[2]:.4f}(σ₃σ_d)^{{{popt[3]:.4f}}} + {popt[4]:.4f}σ_d^{{{popt[5]:.4f}}}$$"
        result.update({
