...
def calcular_modelo(df, model_type, degree):
    ...
    if model_type.startswith("Polinomial"):
        ...
        pfeat = Xp.shape[1]
        r2_adj = adjusted_r2(r2, len(y), pfeat) if len(y) > pfeat+1 else r2
        ...
        if fit_int:
            ...
            intercept = 0.0
        result.update({
            "eq_latex": eq, "intercept": intercept,
            "r2": r2, "r2_adj": r2_adj,
            "rmse": rmse, "mae": mae,
    ...
    # Pezo
        ...
        k10 = y.mean()/(0.101325*(X[:,0]/0.101325).mean()*(X[:,1]/0.101325).mean())
        popt,_=curve_fit(pezo,X,y,p0=[k10,1,1],maxfev=200000)
        y_pred=pezo(X,*popt)
        r2 = r2_score(y,y_pred)
        r2_adj = adjusted_r2(r2,len(y),len(popt)) if len(y)>len(popt)+1 else r2
        rmse = np.sqrt(mean_squared_error(y,y_pred))
