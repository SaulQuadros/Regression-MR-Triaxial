...
def build_latex_equation(coefs, intercept, feature_names):
    """
    Monta equação LaTeX para modelo polinomial, evitando "+ -" 
    e quebrando linhas após cada 4 termos.
    """
    terms_per_line = 4
    # intercept term
    inside = [f"{intercept:.4f}"]
    # coefficients terms
    for coef, term in zip(coefs[1:], feature_names[1:]):
        if coef >= 0:
            inside.append(f"+ {coef:.4f}{term}")
        else:
            inside.append(f"- {abs(coef):.4f}{term}")
    # group into lines
    lines = []
    for i in range(0, len(inside), terms_per_line):
        chunk = inside[i:i+terms_per_line]
...
def build_latex_equation_no_intercept(coefs, feature_names):
    """Monta equação LaTeX para modelo sem intercepto."""
    # prepend zero intercept placeholder
...
def evaluate_quality(y, rmse, mae):
    ...
    return {
        "NRMSE":   (nrmse, qual_nrmse, "NRMSE: RMSE normalizado pela amplitude dos valores de MR; indicador associado ao RMSE."),
        "CV(RMSE)": (cv_rmse, qual_cv, "CV(RMSE): coeficiente de variação do RMSE; indicador associado ao RMSE."),
...
def calcular_modelo(df, model_type, degree):
    ...
    if model_type.startswith("Polinomial"):
        ...
        if fit_int:
            ...
            intercept = 0.0
        result.update({
            "eq_latex": eq_latex, "intercept": intercept,
            "r2": r2, "r2_adj": r2_adj,
            "rmse": rmse, "mae": mae,
...
- ")
        ...
        mae  = mean_absolute_error(y, y_pred)
        result.update({
            "eq_latex": eq_latex, "intercept": 0.0,
            "r2": r2, "r2_adj": r2_adj,
            "rmse": rmse, "mae": mae,
            "mean_MR": y.mean(), "std_MR": y.std(),
    ...
    # Pezo
        ...
        popt, _ = curve_fit(pezo, X, y, p0=[y.mean()/(0.101325*(X[:,0]/0.101325).mean()*(X[:,1]/0.101325).mean()),1,1], maxfev=200000)
        eq = f"$$MR = {popt[0]*0.101325:.4f}(σ₃/0.101325)^{{{popt[1]:.4f}}}(σ_d/0.101325)^{{{popt[2]:.4f}}}$$"
        inside = eq.strip("$$")
        inside = inside.replace(" + ", " \\
+ ")
        inside = inside.replace(" - ", " \\
