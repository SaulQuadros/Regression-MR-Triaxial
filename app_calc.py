...
def evaluate_quality(y, rmse, mae):
    ...
    return {
        "NRMSE_range": (nrmse, qual_nrmse, "NRMSE: RMSE normalizado pela amplitude dos valores de MR."),
        "CV(RMSE)":    (cv_rmse, qual_cv, "CV(RMSE): RMSE dividido pela média de MR."),
        "MAE %":       (mae_pct, qual_mae, "MAE %: MAE dividido pela média de MR.")
    }
...
def calcular_modelo(df, model_type, degree):
    ...
    if model_type in CLASSICOS:
        ...
        # Equação genérica fallback
        params = ", ".join(f"{v:.4f}" for v in popt)
        eq = f"$$MR = ({params})$$"
        result.update({
            "eq_latex": eq,
            "intercept": 0.0, "r2": r2, "r2_adj": r2_adj,
    ...
    if model_type.startswith("Polinomial"):
        ...
        result.update({
            "eq_latex": eq, "intercept": reg.intercept_, "r2": r2, "r2_adj": r2_adj,
            "rmse": rmse, "mae": mae, "mean_MR": y.mean(), "std_MR": y.std(),
